...
class Cms_Controller extends MX_Controller {
    ...
    public $cmsJSextra = array(); // arqvs JS inseridos extra controller
...
//            mybug($this->dados['metadados']);
     ...
     */
    private function setNewScript($array) {
        if (!is_array($array))
            $array = explode(',', $array);
        $this->cmsJSextra = array_merge($this->cmsJSextra, $array);
    }
    
    private function setNewJquery($array) {
        if (!is_array($array))
            $array = explode(',', $array);
    ...
    private function setNewEstyle($array) {
        if (!is_array($array))
            $array = explode(',', $array);
