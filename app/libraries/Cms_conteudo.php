 ...
 * $config['append'] = array(
            array(
                'titulo' => 'Fale conosco', 'uri' => 'contato/faleconosco'
            ),
            array(
...
class Cms_conteudo{
    ...
    public function get_page_gallery($page_nick_id = NULL){
        ...
        $return = $this->parse_gallery($return);
     ...
     */
    private function parse_gallery($attach_array){
        return $attach_array;
    }
...
//        mybug($this->modulo_id);
    ...
    private function parse_html($builded_menu){
        ...
        $this->template_html[] = '<ul '.$this->get_id_or_class($this->ul_class).'>';
    ...
    private function parse_html_recursive($builded_menu, $parent_grupo, $level_flag){
        ...
        $this->template_html[] = '<ul class="children children-'.$level_flag.'">';
    ...
    // -------------------------------------------------------------------------
    /**
     * Recebe string, identifica se é classe ou ID e retorna atributo.
     * 
     * @param type $class
     * @return string 
     */
    private function get_id_or_class($class){
        
        $symbol = substr(trim($class), 0, 1);
        $symbol = substr(trim($class), 0, 1);
        
        if($symbol == '#'){
            $class = substr(trim($class), 1);
            $ret = 'id="';
        } else if($symbol == '.') {
            $class = substr(trim($class), 1);
            $ret = 'class="';
