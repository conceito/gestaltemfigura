...
class Cms_extrato {
     ...
     * @param type $dados
     */
    public function status_update($metodo = 'pagseguro', $dados = array()){
        ...
        $extrato = $this->get($dados['reference']);
        
        // atualiza status ... apenas
        $dados['anotacao'] = $dados['status'];
        $dados['status'] = $dados['code'];
        
        $ret = $this->ci->db->update('cms_extratos', $dados, array('id' => $dados['reference']));
        
        // salva no histórico
        $this->history_update(array(
            'extrato_id' => $extrato['id'],
            'anotacao'   => $dados['status'],
            'status'   => $dados['code']
        ));
