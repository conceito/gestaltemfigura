 ...
 * $posts = $this->cms_posts->get(array(
            'modulo_id' => int,         // padrão 7
            'grupo_id' => false|array()|int, // padrão false
            'per_page' => int,          // padrão 100
 *          'start_page'  => false,     // sobrepõe 'pag' da uri
            'ignore_pagination' => false,// pega a partir do indice 0 (zero)
 *          'base_url' => string,       // padrão inicio/index (para paginação)
 *          'post_uri' => string        // padrão 'p/[grupo]/[nick]' (controller do conteúdo)
 *          'pag' => false|int,         // por padrão resgata "pag" da URI
 *          'status' => 0|1|2,          // padrão 1
 *          'ordem' => string,          // padrão 'cont.dt_ini desc'
 *          'campos' => string,         // padrão 'id, nick, titulo, resumo, dt_ini, galeria, modulo_id'
 *          'dt_ini' => date,           // padrão false
 *          'lang' => string,           // padrão 'pt', se for Multilamg resgata a variável $this->lng
 *          'tipo' => string,           // padrão 'conteudo'
 *          'destaque' => false|0|1,    // padrão false
 *          'ignore' => array,          // padrão false
 *          'gallery_tag' => false|1|2|3|4|5, // padrão 1
 *          'gallery_no_tag' => false|1|2|3|4|5, // padrão false
 *          'tag' => int,               // padrão false
 *          'gettags' => bool           // padrão false
 * 
        ));
...
class Cms_posts{
    ...
    public function get($post = false){
        ...
        if($this->status !== false) $this->ci->db->where('cont.status', $this->status);
