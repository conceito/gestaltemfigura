...
//        
//        $this->ci->load->library(array('site_utils', 'cms_conteudo'));
//        
//        $config['home'] = false;
//        $config['modulo_id'] = 6;
//        $config['html'] = false;
...
//        // monta a hierarquia das páginas
//        $all_pages = $this->ci->cms_conteudo->generate_menu($config);
        
        $all_pages = $this->ci->paginas_model->get_arquitetura(false);
        
//        mybug($all_pages['rows']);
        $data[] = '<?php ';
        
        // for every page in the database, get the route using the recursive function - _get_route()
        foreach( $all_pages['rows'] as $route )
        {
            $data[] = '$route["' . $route['uri'] . '"] = "' . "pages/display/{$route['id']}" . '";';
            // caso a uri tenha mais de um segmento, escreve o nick da página
            // para evitar erros de roteamento
            $urilevel = explode('/', $route['uri']);
            if(count($urilevel) > 1){
               $data[] = '$route["' . $route['nick'] . '"] = "' . "pages/display/{$route['id']}" . '";'; 
            }
...
//        $idGrupo = (!isset($rels2[1])) ? 0 : $rels2[1]; # deprecated
        ...
        if($rels != 25){
            ...
            $this->ci->db->join('cms_modulos as modulo', 'modulo.id = conteudo.modulo_id');
            $this->ci->db->join('cms_conteudo as grupo', 'grupo.id = conteudo.grupo');
            
            
            
            $this->ci->db->distinct();
            foreach($idMod as $modulo_id){
                $this->ci->db->or_where('conteudo.modulo_id', $modulo_id);
            }
